...
export async function processOutbox(): Promise<void> {
  try {
    ...
    logger.debug('Processing outbox events', { count: unpublishedEvents.length });
    ...
    for (const event of unpublishedEvents) {
      try {
        ...
        // Map event type to routing key (e.g., 'visitor_pass.created' -> 'visitor_pass.created.v1')
...
export function startOutboxWorker(): void {
  logger.info('Starting outbox worker', { intervalMs: POLL_INTERVAL_MS });
