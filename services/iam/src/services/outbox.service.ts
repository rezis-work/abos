  ...
  try {
    ...
    for (const event of unpublishedEvents) {
      try {
        ...
        // Create event payload (remove correlationId from payload as it's now in event metadata)
        const { correlationId: _, ...cleanPayload } = payload;
        ...
        // Map event type to routing key (e.g., 'user.created' -> 'user.created.v1')
